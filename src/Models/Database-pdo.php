...
class Database  {
	...
	public function checkPassword($nickname, $password) {
		$pdo = $this->connection;
		$data = $pdo->quote($nickname);
		$query =  "SELECT password FROM users WHERE nickname=$data";
	...
	public function updateUser($nickname, $password) {
		if ($this->checkPasswordValidity($password)) {
			$pdo = $this->connection;
			$data = $pdo->quote(password_hash($password, PASSWORD_BCRYPT));
			$data2 = $pdo->quote($nickname);
			$query =  "UPDATE users SET password=$data WHERE nickname=$data2";
			$cpt = $pdo->exec($query);
	...
	public function loadSurveysByKeyword($keyword) {
		...
		$pdo = $this->connection;
		$data = $pdo->quote('%'.$keyword.'%');
		$query =  "SELECT owner, question FROM surveys WHERE question LIKE $data";
		//var_dump($query);die;
		$stmt = $pdo->query($query);
		...
		if(!empty($rows)){
			foreach($rows as $row){
				foreach($row as $data){
					$result[] = new Survey($data["owner"],$data["question"]);
				}
			return $result;
		}};
		$pdo = null;
